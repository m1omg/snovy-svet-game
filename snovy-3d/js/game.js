...
const DIFFICULTY = {
    EASY:   { name: 'Ľahká',  lives: 15, enemyBase: 2, enemyPerRealm: 1.0, enemySpeed: 0.7, invinc: 2.5, enemySleepTime: 20, bossHpMul: 0.7, jumpMul: 1.0 },
    MEDIUM: { name: 'Stredná', lives: 12, enemyBase: 3, enemyPerRealm: 1.6, enemySpeed: 1.0, invinc: 1.8, enemySleepTime: 12, bossHpMul: 1.0, jumpMul: 1.0 },
    HARD:   { name: 'Ťažká',  lives: 10, enemyBase: 5, enemyPerRealm: 2.4, enemySpeed: 1.4, invinc: 1.2, enemySleepTime: 7, bossHpMul: 1.3, jumpMul: 1.0 },
    CUSTOM: { name: 'Vlastná', lives: 12, enemyBase: 3, enemyPerRealm: 1.6, enemySpeed: 1.0, invinc: 1.8, enemySleepTime: 12, bossHpMul: 1.0, jumpMul: 1.0 }
};
...
const CUSTOM_PARAMS = [
    ...
    { key: 'jumpMul',        min: 0.6, max: 2.0, step: 0.1, dec: 1 }
];
...
function updateBoss(dt) {
    ...
    if (dist < boss.radius + 1.0) {
        if (player.vel.y < -2 && player.pos.y > boss.y + 1.0 && boss.invincible <= 0) {
            damageBoss(22, true);
            player.vel.y = CONFIG.JUMP_VELOCITY * 1.05 * (currentDifficulty.jumpMul || 1);             // väčší odraz pri dupnutí na bossa
...
function tryJump() {
    if (player.jumpsLeft > 0) {
        player.vel.y = CONFIG.JUMP_VELOCITY * (mobileMode ? 1.25 : 1) * (currentDifficulty.jumpMul || 1);   // mobilný režim = vyšší skok; jumpMul = obtiažnosť
        player.jumpsLeft--;
        player.onGround = false;
...
function updateBeam(dt) {
    ...
    if (wantBeam) {
        ...
        else if (!isTouchDevice && (lmbBeam || actionHeld('beam'))) ensureMouseBeamAim();
...
function computeBeamDir() {
    ...
    if (!isTouchDevice && !mobileMode) return _bd.set(hx, 0, hz).normalize();
    ...
    for (const e of enemies) {
        ...
        if (e.type === 'ghost') {
            ...
            if (e.onGround) {
                ...
                // pristane NA najvyššej plošine pod sebou (neprepadne cez ňu); baseY/groundY sa aktualizujú
                const restOff = e.baseY - e.groundY;
                let landTop = null;
                if (e.vy < 0) for (const p of platforms) {
                    if (e.mesh.position.x < p.min.x || e.mesh.position.x > p.max.x) continue;
                    if (e.mesh.position.z < p.min.z || e.mesh.position.z > p.max.z) continue;
                    const restY = p.top + restOff;
                    if (prevY >= restY - 0.02 && e.mesh.position.y <= restY && (landTop === null || p.top > landTop)) landTop = p.top;
                }
                if (landTop === null && e.mesh.position.y <= e.baseY) landTop = e.baseY - restOff;   // záchrana
                if (landTop !== null) {
                    e.groundY = landTop; e.baseY = landTop + restOff;
...
function checkEnemyPlayer(e) {
    ...
    if (player.vel.y < 0 && player.pos.y > e.mesh.position.y - 0.3) {
        ...
        player.vel.y = CONFIG.JUMP_VELOCITY * (0.95 + chain * 0.045) * (currentDifficulty.jumpMul || 1); // väčší odraz, rastie s combom
...
function loadCustomDiff() {
    try {
        ...
        if (s) for (const p of CUSTOM_PARAMS) if (typeof s[p.key] === 'number') DIFFICULTY.CUSTOM[p.key] = Math.max(p.min, Math.min(p.max, s[p.key]));
    } catch (e) { /* ignoruj */ }
}
...
function resetCustomDiff() {
    for (const p of CUSTOM_PARAMS) DIFFICULTY.CUSTOM[p.key] = DIFFICULTY.MEDIUM[p.key];
    saveCustomDiff(); buildCustomDiff();
}
function buildCustomDiff() {
    ...
    for (const p of CUSTOM_PARAMS) {
        ...
        const lab = document.createElement('span'); lab.className = 'ctrl-label'; lab.textContent = labels[p.key] || p.key;
        ...
        val.textContent = DIFFICULTY.CUSTOM[p.key].toFixed(p.dec);
        const plus = document.createElement('button'); plus.className = 'ctrl-key'; plus.textContent = '+'; plus.style.minWidth = '40px';
        minus.addEventListener('click', () => adjustCustom(p, -1));
        plus.addEventListener('click', () => adjustCustom(p, 1));
...
function setupMenus() {
    ...
    document.querySelectorAll('.cap-toggle').forEach(b => b.addEventListener('click', () => setCaptureMouse(!captureMouse)));
