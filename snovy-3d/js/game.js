...
const RMB_HOLD_MS = 200;             // hranica klik vs. držanie pravého tlačidla
...
function updateCamera(dt) {
    ...
    _camTarget.set(
        player.pos.x + Math.sin(camYaw) * cp * CAM_RADIUS,
        player.pos.y + sp * CAM_RADIUS,
        player.pos.z + Math.cos(camYaw) * cp * CAM_RADIUS
    );
...
function setupMouse() {
    ...
    window.addEventListener('mousedown', (e) => {
        ...
        if (e.button === 2) { rmbDown = true; rmbDownT = performance.now(); rmbMoved = false; rmbAcc = 0; }  // pravé: kamera / klik = skok
        else if (e.button === 0) lmbBeam = true;                                                              // ľavé: baterka
        else if (actionHasCode('jump', 'Mouse' + e.button)) requestJump();
    });
    window.addEventListener('mousemove', (e) => {
        if (!rmbDown || currentState !== GameState.PLAYING) return;        // kameru otáča iba držané pravé tlačidlo
        camYaw -= e.movementX * 0.005;
        camPitch = Math.max(0.12, Math.min(1.4, camPitch - e.movementY * 0.005));
        rmbAcc += Math.abs(e.movementX) + Math.abs(e.movementY);
        if (rmbAcc > 6) rmbMoved = true;
    });
