...
let cameraMode = 'free';             // 'free' = voľný orbit (myš), 'spyro' = vlečená kamera za chrbtom (auto-stáčanie)
...
function updateCamera(dt) {
    // Spyro režim: vlečená kamera sa plynule stáča za chrbát hráča (smer pohybu/pohľadu)
    if (cameraMode === 'spyro' && currentState === GameState.PLAYING && !pointerLocked) {
        const fx = player.faceDir.x, fz = player.faceDir.z;
        if (fx * fx + fz * fz > 1e-4) {
            const targetYaw = Math.atan2(-fx, -fz);   // za chrbtom hráča
            let dy = targetYaw - camYaw;
            while (dy > Math.PI) dy -= Math.PI * 2;
            while (dy < -Math.PI) dy += Math.PI * 2;
            const moving = (Math.abs(player.vel.x) + Math.abs(player.vel.z)) > 0.6;
            camYaw += dy * (1 - Math.exp(-(moving ? 4.5 : 1.6) * dt));   // rýchlejšie počas pohybu, lenivé v pokoji
        }
        camPitch += (CAM_PITCH0 - camPitch) * (1 - Math.exp(-2.5 * dt));   // jemne sa vráti na pohodlný uhol
    }
...
function updateLanguage() {
    ...
    setText('cameraLabel', L('cameraLabel'));
    setText('pauseCameraLabel', L('cameraLabel'));
    setText('camFreeMenu', L('camFree')); setText('camSpyroMenu', L('camSpyro'));
    setText('camFreePause', L('camFree')); setText('camSpyroPause', L('camSpyro'));
...
// ---------- Kamerový režim (voľná / Spyro) ----------
function setCameraMode(mode) {
    if (mode !== 'free' && mode !== 'spyro') return;
    cameraMode = mode;
    try { localStorage.setItem('snovy3d_cameramode', cameraMode); } catch (e) { /* ignoruj */ }
    document.querySelectorAll('.cam-btn[data-cam]').forEach(b => b.classList.toggle('active', b.dataset.cam === mode));
}
function loadCameraMode() { try { const m = localStorage.getItem('snovy3d_cameramode'); if (m === 'free' || m === 'spyro') cameraMode = m; } catch (e) { /* ignoruj */ } }
...
function setupMenus() {
    ...
    document.querySelectorAll('.cam-btn[data-cam]').forEach(b => b.addEventListener('click', () => setCameraMode(b.dataset.cam)));
