...
const DEFAULT_BINDINGS = {
    ...
    jump:  ['Space', 'Mouse2'],          // medzerník + pravé tlačidlo myši
...
function updateBeam(dt) {
    const wantBeam = (actionHeld('beam') || touchBeamHeld || dpadBeamHeld) && player.beamEnergy > 0;
    if (wantBeam) {
...
// ---------- Myš (PC): ľavé tlačidlo = otáčanie kamery, pravé = skok (predvolene) ----------
function setupMouse() {
    document.addEventListener('contextmenu', (e) => e.preventDefault());   // pravé tlačidlo neотvára kontext. menu
    let dragging = false;
    window.addEventListener('mousedown', (e) => {
        ...
        if (actionHasCode('jump', 'Mouse' + e.button)) requestJump();
        if (e.button === 0) dragging = true;                               // ľavé = ťahaj kameru
    });
    window.addEventListener('mousemove', (e) => {
        if (!dragging || currentState !== GameState.PLAYING) return;
