...
const DEFAULT_BINDINGS = {
    ...
    jump: 'Space', beam: 'ControlLeft', pause: 'KeyP'
};
let controlBindings = Object.assign({}, DEFAULT_BINDINGS);
let rebinding = null;            // akcia, ktorej práve priraďujeme kláves (alebo null)
let controlsReturn = 'menuScreen';

function loadBindings() {
    try {
        const s = JSON.parse(localStorage.getItem('snovy3d_controls'));
        if (s) for (const a of CONTROL_ACTIONS) if (typeof s[a] === 'string') controlBindings[a] = s[a];
...
// Pekný názov klávesu pre tlačidlo
...
function disposeObject(obj) {
    obj.traverse(o => {
        // POZOR: THREE.Sprite zdieľa jednu globálnu geometriu medzi VŠETKÝMI spritmi.
        // Disponovanie jej buffera pri prepnutí režimu rozbilo ostatné billboardy/halo → mizli nepriatelia.
...
// Kontaktný (blob) tieň pod nepriateľom v 2D billboard režime (sprite nevie vrhať tieň)
function updateEnemyShadow(e) {
    if (visualMode !== 'billboard' || e.dead) { if (e.shadowMesh) e.shadowMesh.visible = false; return; }
    if (!e.shadowMesh) {
    ...
    e.shadowMesh.position.set(e.mesh.position.x, e.groundY + 0.04, e.mesh.position.z);
    const r = (e.radius || 1) * 2.0;
    e.shadowMesh.scale.set(r, r, 1);
    const h = Math.max(0, e.mesh.position.y - e.groundY);   // vyššie = bledší/menší tieň
    e.shadowMesh.material.opacity = Math.max(0.08, 0.42 - h * 0.025);
}
...
function updateBoss(dt) {
    ...
    if (dist < boss.radius + 1.0) {
        if (player.vel.y < -2 && player.pos.y > boss.y + 1.0 && boss.invincible <= 0) {
            damageBoss(22, true);
            player.vel.y = CONFIG.JUMP_VELOCITY * 1.05;             // väčší odraz pri dupnutí na bossa
...
function updatePlayer(dt) {
    ...
    let ix = 0, iz = 0;
    // klávesnica — osi sa sčítavajú a nižšie normalizujú, takže diagonála (napr. ↑+→) funguje
    if (keys[controlBindings.up]) iz -= 1;
    if (keys[controlBindings.down]) iz += 1;
    if (keys[controlBindings.left]) ix -= 1;
    if (keys[controlBindings.right]) ix += 1;
...
function tryJump() {
    if (player.jumpsLeft > 0) {
        player.vel.y = CONFIG.JUMP_VELOCITY * (mobileMode ? 1.25 : 1);   // mobilný režim = vyšší skok
        player.jumpsLeft--;
        player.onGround = false;
...
function updateBeam(dt) {
    // Oba Ctrl klávesy ostávajú ekvivalentné, keď je baterka mapovaná na Ctrl (inak presný kód)
    const keyBeam = (controlBindings.beam === 'ControlLeft' || controlBindings.beam === 'ControlRight')
        ? (keys['ControlLeft'] || keys['ControlRight'] || keys['Control'])
        : keys[controlBindings.beam];
    const wantBeam = (keyBeam || touchBeamHeld || dpadBeamHeld) && player.beamEnergy > 0;
    if (wantBeam) {
        // dotykové mierenie: lúč smeruje tam, kam ukazuje prst (a hladko ho zametá)
...
// Premietne dotykový bod na vodorovnú rovinu vo výške pôvodu lúča a nasmeruje naň lúč (XZ)
function aimBeamAtScreen(sx, sy) {
...
// Smer lúča: horizontálne podľa vstupu, vertikálne sa AUTOMATICKY dosmeruje na najbližšieho
// nepriateľa v prednom oblúku (preto baterka „dosiahne" aj lietajúcich/nižšie položených).
...
function checkEnemyPlayer(e) {
    ...
    if (player.vel.y < 0 && player.pos.y > e.mesh.position.y - 0.3) {
        ...
        player.vel.y = CONFIG.JUMP_VELOCITY * (0.95 + chain * 0.045); // väčší odraz, rastie s combom
...
function buildControlsMenu() {
    ...
    for (const a of CONTROL_ACTIONS) {
        ...
        const btn = document.createElement('button'); btn.className = 'ctrl-key'; btn.dataset.action = a;
        if (rebinding === a) { btn.textContent = L('pressKey'); btn.classList.add('listening'); }
        else btn.textContent = keyLabel(controlBindings[a]);
        btn.addEventListener('click', () => { rebinding = a; buildControlsMenu(); });
        row.appendChild(lab); row.appendChild(btn); list.appendChild(row);
    }
}
function applyRebind(code) {
    if (!rebinding) return;
    // žiadne duplicity: kód odober z inej akcie, ak ho mala
    for (const a of CONTROL_ACTIONS) if (a !== rebinding && controlBindings[a] === code) controlBindings[a] = '';
    controlBindings[rebinding] = code;
    rebinding = null;
    saveBindings();
    buildControlsMenu();
}
function resetControls() {
    controlBindings = Object.assign({}, DEFAULT_BINDINGS);
    rebinding = null;
    saveBindings();
    buildControlsMenu();
}
...
function setupInput() {
    window.addEventListener('keydown', (e) => {
        ...
        if (['ArrowUp', 'ArrowDown', 'ArrowLeft', 'ArrowRight', 'Space'].includes(e.code) ||
            Object.values(controlBindings).includes(e.code)) e.preventDefault();

        if (currentState === GameState.PLAYING) {
            if (e.code === controlBindings.jump && !e.repeat) requestJump();
            if (e.code === controlBindings.pause || e.code === 'Escape') togglePause();
...
function setupMenus() {
    ...
    document.querySelectorAll('.mm-toggle').forEach(b => b.addEventListener('click', () => setMobileMode(!mobileMode)));
