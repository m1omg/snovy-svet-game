...
const themes = [
    { name: 'Fialová Hmla',            bg1: '#0a0015', bg2: '#1a0a2e', platform: '#4a2080', platformTexture: 'purpleMist', platformStyle: 'forcefield', accent: '#b060ff', musicScale: 'ethereal',   decor: 'mist',      decorColor: '#8040c0' },
    { name: 'Zabudnutá Izba Hračiek',  bg1: '#1a1525', bg2: '#2a2035', platform: '#8a6050', platformTexture: 'toyRoom', accent: '#ffb080', musicScale: 'melancholy', decor: 'toys',      decorColor: '#ff9060' },
    { name: 'Čiernobiely Labyrint',    bg1: '#080808', bg2: '#151515', platform: '#404040', platformTexture: 'blackWhiteLabyrinth', accent: '#ffffff', musicScale: 'cosmic',     decor: 'maze',      decorColor: '#606060' },
    { name: 'Potopený Chrám',          bg1: '#001520', bg2: '#003040', platform: '#205060', platformTexture: 'sunkenTemple', accent: '#40d0ff', musicScale: 'ethereal',   decor: 'bubbles',   decorColor: '#60c0e0' },
    { name: 'Cukríkové Nebo',          bg1: '#200820', bg2: '#401040', platform: '#c06080', platformTexture: 'candySky', accent: '#ff80c0', musicScale: 'melancholy', decor: 'candy',     decorColor: '#ff60a0' },
    { name: 'Tichý Les Tieňov',        bg1: '#000a05', bg2: '#001a10', platform: '#203020', platformTexture: 'shadowForest', accent: '#40a060', musicScale: 'cosmic',     decor: 'trees',     decorColor: '#306040' },
    { name: 'Rozbitý Čas',             bg1: '#0a0510', bg2: '#150a20', platform: '#503060', platformTexture: 'brokenTime', accent: '#c080ff', musicScale: 'ethereal',   decor: 'clocks',    decorColor: '#a060d0' },
    { name: 'Plyšový Svet',            bg1: '#151015', bg2: '#252025', platform: '#806050', platformTexture: 'plushWorld', accent: '#ffc090', musicScale: 'melancholy', decor: 'plush',     decorColor: '#d0a080' },
    { name: 'Prázdny Cirkus',          bg1: '#100508', bg2: '#200a10', platform: '#602030', platformTexture: 'emptyCircus', accent: '#ff4060', musicScale: 'cosmic',     decor: 'circus',    decorColor: '#c03050' },
    { name: 'Hviezdy a Mesiace',       bg1: '#020008', bg2: '#050015', platform: '#303060', platformTexture: 'starsMoons', accent: '#8080ff', musicScale: 'ethereal',   decor: 'cosmos',    decorColor: '#6060c0' },
    { name: 'Zabudnutý Domov',         bg1: '#100a08', bg2: '#201510', platform: '#504030', platformTexture: 'forgottenHome', accent: '#c0a080', musicScale: 'melancholy', decor: 'home',      decorColor: '#907050' },
    { name: 'Prebudenie',              bg1: '#150810', bg2: '#301020', platform: '#603050', platformTexture: 'awakening', accent: '#ff60ff', musicScale: 'cosmic',     decor: 'awakening', decorColor: '#d040c0' }
];
...
const PLATFORM_TEXTURE_VERSION = '35-textured';
const PLATFORM_TEXTURE_PATHS = {
    purpleMist: 'assets/platform-purple-mist.png',
    toyRoom: 'assets/platform-toy-room.png',
    blackWhiteLabyrinth: 'assets/platform-black-white-labyrinth.png',
    sunkenTemple: 'assets/platform-sunken-temple.png',
    candySky: 'assets/platform-candy-sky.png',
    shadowForest: 'assets/platform-shadow-forest.png',
    brokenTime: 'assets/platform-broken-time.png',
    plushWorld: 'assets/platform-plush-world.png',
    emptyCircus: 'assets/platform-empty-circus.png',
    starsMoons: 'assets/platform-stars-moons.png',
    forgottenHome: 'assets/platform-forgotten-home.png',
    awakening: 'assets/platform-awakening.png'
};
...
const BUILD_ID = 'build 36-textured';
let frameCount = 0;
let lastKeyCode = '–';
...
function loadPlatformTextures() {
    ...
    for (const key in PLATFORM_TEXTURE_PATHS) {
        const path = PLATFORM_TEXTURE_PATHS[key] + '?v=' + PLATFORM_TEXTURE_VERSION;
        const tex = loader.load(path, () => { tex.needsUpdate = true; }, undefined,
...
function addPlatform(x, y, z, w, d, theme) {
    ...
    const textured = !!textureMap;
    const forcefield = textured && theme.platformStyle === 'forcefield';
    const mat = new THREE.MeshStandardMaterial({
        color: textured ? (forcefield ? 0xf0d6ff : 0xffffff) : new THREE.Color(theme.platform),
        map: textureMap,
        emissiveMap: forcefield ? textureMap : null,
        emissive: new THREE.Color(theme.accent).multiplyScalar(forcefield ? 0.5 : (textured ? 0.08 : 0.12)),
        emissiveIntensity: forcefield ? 1.35 : 1.0,
        transparent: forcefield,
        opacity: forcefield ? 0.72 : 1.0,
        depthWrite: !forcefield,
        roughness: forcefield ? 0.24 : (textured ? 0.82 : 0.85),
        metalness: forcefield ? 0.0 : (textured ? 0.02 : 0.05)
    });
    const mesh = new THREE.Mesh(geo, mat);
    mesh.position.set(x, y, z);
    mesh.castShadow = !forcefield;       // forcefield plošiny majú pôsobiť ako svetlo, nie tmavý masív
    mesh.receiveShadow = !forcefield;
    scene.add(mesh);
...
function makeEnemyModel(type) {
    ...
    if (type === 'ghost') {
        ...
        // 3D: rôsolovitá améba — nepravidelná priesvitná zelená hmota (prekrývajúce sa laloky), pseudonôžky,
        // presvitajúce jadro + vakuoly a jedno veľké oko. (Predtým plochý „struk" – teraz organický blob.)
        const jelly = std(0x63bf63, 0x2f7b39, 0.35, 0.86);          // priesvitné zelené „želé"
        const blob = (x, y, z, r, sx, sy, sz) => {
            const m = new THREE.Mesh(new THREE.SphereGeometry(r, 16, 12), jelly);
            m.position.set(x, y, z); m.scale.set(sx === undefined ? 1 : sx, sy === undefined ? 0.8 : sy, sz === undefined ? 1 : sz);
            group.add(m); return m;
        };
        // hlavná hmota: prekrývajúce sa laloky rôznej veľkosti → organický hrboľatý tvar (nie hladký struk)
        blob(0, 0, 0, 0.95);
        blob(0.7, -0.05, 0.06, 0.6); blob(-0.66, 0.06, -0.06, 0.62);
        blob(-0.34, -0.06, -0.46, 0.5); blob(0.36, 0.09, 0.34, 0.48);
        blob(0.05, 0.36, -0.04, 0.45); blob(0.12, -0.3, 0.12, 0.5);
        // pseudonôžky: nepravidelné výbežky do strán
        blob(1.02, -0.1, 0.24, 0.26, 1.25, 0.55, 0.85);
        blob(-0.98, 0.02, 0.3, 0.24, 1.2, 0.6, 0.9);
        blob(0.22, -0.12, -0.9, 0.26, 0.9, 0.6, 1.2);
        blob(-0.55, 0.12, 0.72, 0.22, 0.9, 0.6, 1.1);
        for (let i = 0; i < 3; i++) {                               // pár drobných náhodných hrbolčekov → každá améba trochu iná
            const a = Math.random() * Math.PI * 2, rr = 0.78 + Math.random() * 0.32;
            blob(Math.cos(a) * rr, (Math.random() - 0.5) * 0.35, Math.sin(a) * rr * 0.8, 0.15 + Math.random() * 0.12, 1, 0.6, 0.9);
        }
        // priesvitná membrána (vonkajší žiariaci obal)
        const shell = new THREE.Mesh(new THREE.SphereGeometry(1.1, 20, 16), basic(0x9be88f, 0.16));
        shell.scale.set(1.22, 0.95, 1.1); group.add(shell);
        // jadro (nucleus) + vakuoly — presvitajú cez želé
        const nucleus = new THREE.Mesh(new THREE.SphereGeometry(0.36, 16, 12), std(0x2e7d3a, 0x123018, 0.2, 0.96));
        nucleus.position.set(0.16, -0.04, 0.0); nucleus.scale.set(1, 0.85, 1); group.add(nucleus);
        for (const v of [[-0.4, 0.18, 0.28, 0.14], [0.46, 0.2, -0.08, 0.11]]) {
            const vac = new THREE.Mesh(new THREE.SphereGeometry(v[3], 12, 10), basic(0xcaf5b8, 0.5));
            vac.position.set(v[0], v[1], v[2]); group.add(vac);
        }
        // veľké oko + zrenica (vpredu, mierne nabok)
        const eye = new THREE.Mesh(new THREE.SphereGeometry(0.32, 18, 14), std(0x1f4f22, 0x0e260e, 0.1, 0.99));
        eye.position.set(-0.18, 0.12, 0.74); eye.scale.set(1, 0.92, 0.62); group.add(eye);
        const pupil = new THREE.Mesh(new THREE.SphereGeometry(0.15, 12, 10), basic(0x0a1f0a));
        pupil.position.set(-0.24, 0.1, 0.95); pupil.scale.set(1, 0.92, 0.5); group.add(pupil);
