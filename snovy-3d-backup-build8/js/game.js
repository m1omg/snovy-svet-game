...
function updateBeam(dt) {
    ...
    const beamDown = (controlBindings.beam === 'ControlLeft' || controlBindings.beam === 'ControlRight')
        ? (keys['ControlLeft'] || keys['ControlRight'] || keys['Control'])
        : keys[controlBindings.beam];
    const wantBeam = beamDown && player.beamEnergy > 0;
    if (wantBeam) {
...
function setupInput() {
    window.addEventListener('keydown', (e) => {
        ...
        if (currentState === GameState.PLAYING) {
            if (e.code === controlBindings.jump && !e.repeat) tryJump();
